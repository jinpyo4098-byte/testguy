...
# 스트림릿 페이지 설정 및 여백 제거
st.set_page_config(page_title="Gravity Arrow - Fixed Edition", layout="wide")
st.markdown(
    """
    <style>
...
<head>
    ...
    <title>Gravity Arrow - Ultimate Fixed</title>
    <style>
        body, html {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <script>
        ...
        function updateLoop() {
            ...
            envParticles.forEach(p => {
                ...
                if (currentPlanetKey === 'earth') {
                    ...
                    ctx.fillStyle = 'rgba(150, 105, 15, 0.55)'; ctx.fillRect(p.x, p.y, p.w, p.h);
                } else if (currentPlanetKey === 'europa') {
                    ctx.fillStyle = 'rgba(255, 255, 255, 0.88)'; ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI*2); ctx.fill();
                }
            });
            ...
            if (currentPlanetKey === 'earth') {
                ...
                ctx.fillStyle = '#228b22'; ctx.fillRect(0, gY, canvas.width, 16);
            } else if (currentPlanetKey === 'moon') {
                ctx.fillStyle = '#444444'; ctx.beginPath(); ctx.moveTo(0, gY);
                for(let i=0; i<=canvas.width; i+=40) { ctx.lineTo(i, gY + Math.sin(i * 0.025) * 16); }
                ctx.lineTo(canvas.width, canvas.height); ctx.lineTo(0, canvas.height); ctx.fill();
            } else if (currentPlanetKey === 'mars') {
                ctx.fillStyle = '#8b4513'; ctx.beginPath(); ctx.moveTo(0, gY);
                for(let i=0; i<=canvas.width; i+=50) { ctx.lineTo(i, gY + Math.cos(i * 0.02) * 22); }
                ctx.lineTo(canvas.width, canvas.height); ctx.lineTo(0, canvas.height); ctx.fill();
            } else if (currentPlanetKey === 'venus') {
                ctx.fillStyle = '#b8860b'; ctx.fillRect(0, gY, canvas.width, groundHeight);
            } else if (currentPlanetKey === 'europa') {
            ...
            if(target.visible) {
                ...
                ctx.save();
                ctx.beginPath(); ctx.ellipse(target.x, target.y, target.radiusC * skewX, target.radiusC, 0, 0, Math.PI * 2); ctx.clip(); 

                if (currentPlanetKey === 'earth') {
                    ctx.fillStyle = '#2b82c9'; ctx.fillRect(target.x - 100, target.y - 150, 200, 300);
                    ctx.fillStyle = '#228b22'; ctx.beginPath(); ctx.ellipse(target.x - 10, target.y - 20, 25, 45, Math.PI/6, 0, Math.PI*2); ctx.fill();
                } else if (currentPlanetKey === 'moon') {
                    ctx.fillStyle = '#bbbbbb'; ctx.fillRect(target.x - 100, target.y - 150, 200, 300);
                    ctx.fillStyle = '#555555'; ctx.beginPath(); ctx.arc(target.x - 12, target.y - 30, 14, 0, Math.PI*2); ctx.fill();
                } else if (currentPlanetKey === 'mars') {
                    ctx.fillStyle = '#e03e1d'; ctx.fillRect(target.x - 100, target.y - 150, 200, 300);
                    ctx.fillStyle = '#8b4513'; ctx.beginPath(); ctx.ellipse(target.x - 15, target.y - 15, 16, 25, Math.PI/3, 0, Math.PI*2); ctx.fill();
                } else if (currentPlanetKey === 'venus') {
                    ctx.fillStyle = '#ffd166'; ctx.fillRect(target.x - 100, target.y - 150, 200, 300);
                    ctx.fillStyle = '#b8860b'; ctx.beginPath(); ctx.ellipse(target.x - 5, target.y - 40, 25, 12, 0, 0, Math.PI*2); ctx.fill();
                } else if (currentPlanetKey === 'europa') {
                    ctx.fillStyle = '#a5cad6'; ctx.fillRect(target.x - 100, target.y - 150, 200, 300);
                    ctx.strokeStyle = '#4682b4'; ctx.lineWidth = 2.5; ctx.beginPath(); ctx.moveTo(target.x - 30, target.y - 80); ctx.lineTo(target.x + 30, target.y + 80); ctx.stroke();
                }
                ctx.restore();
            ...
            if(meteor.active) {
                ...
                grad.addColorStop(0, '#ff9e00'); grad.addColorStop(1, '#450a0a');
                ctx.fillStyle = grad; ctx.beginPath(); ctx.arc(meteor.x, meteor.y, meteor.radius, 0, Math.PI*2); ctx.fill();
                ctx.restore();
            }
            ...
            if (arrowTrajectoryVisible && gameActive) {
                ...
                if (tVx > 0) { 
                    ctx.save();
                    ctx.strokeStyle = currentArrow.isGiant ? "#ffcc00" : "rgba(255, 255, 255, 0.6)";
                    ctx.lineWidth = 2.5; ctx.setLineDash([6, 6]); ctx.beginPath();
                    let tX = bowPos.x; let tY = bowPos.y; let tVy = Math.sin(currentAngle) * shootPower;
                    ctx.moveTo(tX, tY);
                    for (let i = 0; i < 60; i++) {
            ...
            for (let i = activeArrows.length - 1; i >= 0; i--) {
                ...
                if(meteor.active) {
                    if(Math.hypot(arrowTipX - meteor.x, arrowTipY - meteor.y) <= meteor.radius + 15) {
                        ...
                        createExplosion(meteor.x, meteor.y, "#ffcc00", 40); activateAbilityBuff(); continue;
                    }
                }
                ...
                if (target.visible && arrowTipX >= frontX && arrowTipX <= backX + 15 && arrow.vx > 0) {
                    ...
                    if (dy <= target.radiusD) {
                        ...
                        createExplosion(arrowTipX, arrowTipY, "#ffcc00", 20);
                        createScoreText(arrowTipX, arrowTipY, `+${earnedPoints}`, "#ffcc00");
                        activeArrows.splice(i, 1); continue;
                    }
                }
            }
            ...
            for (let i = scoreTexts.length - 1; i >= 0; i--) {
                ...
                ctx.save(); ctx.globalAlpha = stx.alpha; ctx.fillStyle = stx.color; font = "bold 22px 'Segoe UI'";
        ...
        function drawArrowIcon(x, y, angle, isApple, isGiant, customWidth) {
            ...
            if (isGiant) {
                ctx.strokeStyle = "#ffcc00"; ctx.lineWidth = 11; ctx.beginPath(); ctx.moveTo(-width/2, 0); ctx.lineTo(width/2, 0); ctx.stroke();
            } else {
                ctx.strokeStyle = isApple ? "#ff3333" : "#e2e8f0"; ctx.lineWidth = 4.5; ctx.beginPath(); ctx.moveTo(-width/2, 0); ctx.lineTo(width/2, 0); ctx.stroke();
